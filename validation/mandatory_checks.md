    ...
    for tag in redis_tags:
        ...
        if key in seen_keys:
            errors.append({
                ...
                'fix': 'Eindeutige Keys für alle Elemente generieren'
            })
        else:
...
def check_W001_performance_optimization(redis_tags):
    ...
    if chunk_sizes:
        ...
        # Sehr große Chunks
        if max_chunk_size > 5000:
            warnings.append({
                'code': 'W001',
                'type': 'WARNING',
                'message': f'Sehr große Chunks erkannt (max: {max_chunk_size} Zeichen)',
                'recommendation': 'Chunks >5000 Zeichen aufteilen für bessere RAG-Performance',
